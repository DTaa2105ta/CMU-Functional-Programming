...
fun leftmost' (Empty: tree): tree = Empty
  | leftmost' (t) =
    ...
    let
      fun helper (Node (Empty, _, tR): tree, fn_acc: tree -> tree) = fn_acc tR
...
fun rightmost' (Empty: tree): tree = Empty
  | rightmost' (t) =
    (* Tail-recursive helper that builds the result tree as it goes *)
  let
    fun helper (Node (tL, _, Empty): tree, fn_acc: tree -> tree) = fn_acc tL
      | helper (Node (tL, c, tR), fn_acc) = 
  ...
  | splitN (t, i) =
    let
      ...
      fun safeLeftmost (t, 0) = t
        ...
        | safeLeftmost (Node(l, x, r), n) =
          safeLeftmost (leftmost' (Node(l, x, r)), n-1)
      
      fun safeRightmost (t, 0) = t
        | safeRightmost (Empty, _) = Empty
        | safeRightmost (Node(l, x, r), n) =
          safeRightmost (rightmost' (Node(l, x, r)), n-1)
    in
      if i <= 0 then (Empty, t)
...
fun leftmost (Empty: tree): char * tree = (#" ", Empty)
  | leftmost (Node(tL, c, tR)) = 
    let
      fun helper (Node(Empty, c', tR'), fn_acc: tree -> tree): char * tree = (c', fn_acc tR')
        | helper (Node(tL, c, tR), fn_acc) =
          helper (tL, fn newLeft => fn_acc (Node(newLeft, c, tR)))
    in
